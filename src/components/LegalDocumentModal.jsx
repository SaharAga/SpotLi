...
import { TERMS_CONTENT, PRIVACY_CONTENT, ACCESSIBILITY_CONTENT } from '../constants/legal';
import { Modal } from './Modal';

/**
 * Read-only viewer for the full Terms of Use, Privacy Policy, or Accessibility Statement text,
 * opened from the registration checkboxes, LegalConsentGate, and AboutModal — one
 * place that renders src/constants/legal.js so the call sites can't
...
export function LegalDocumentModal({ isOpen, onClose, docType = 'terms' }) {
  ...
  const docSource = docMap[docType] || TERMS_CONTENT;
  const content = docSource[language] || docSource.en;
