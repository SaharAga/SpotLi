...
export function InstallGuideDialog({ isOpen, onClose, isIOS, isRTL }) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      componentName="InstallGuideDialog"
      layer="top"
      compact={true}
      labelledBy="ios-guide-title"
      className={`bg-slate-900 border border-slate-700 rounded-2xl p-6 max-w-sm w-full shadow-2xl text-slate-100 ${isRTL ? 'text-right' : 'text-left'}`}
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="text-slate-400 hover:text-slate-100 p-1 rounded-lg min-h-[48px] min-w-[48px] flex items-center justify-center cursor-pointer"
        >
      ...
      <button
        type="button"
        onClick={() => onClose()}
        className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl text-xs transition-colors min-h-[48px] cursor-pointer"
      >
